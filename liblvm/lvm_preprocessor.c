...
static int process_includes(
	char **src, int *src_len, struct lvmhtab_ctx *defines)
{
	...
	FILE *filp = lvmfopen(filename, ".vm", "r");
	...
	size_t addition_len = lvmflength(filp);
	char *addition_str = calloc(addition_len, sizeof(char));

	lvmfcopy(addition_str, addition_len, filp);
	fclose(filp);
	...
	char **src, int *src_len, struct lvmhtab_ctx *defines)
{
	...
	if (lvmhtab_find(defines, keystr) < 0)
		lvmhtab_add_ref(defines, keystr, valstr, strlen(valstr) + 1);
	else {
